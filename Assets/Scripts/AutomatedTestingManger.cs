...
using static AutomatedTestingManger;

public class AutomatedTestingManger : MonoBehaviour
{
    [System.Serializable]
    ...
    void Start()
    {
        evaluatedfilename = Path.Combine(Application.dataPath, "Testcases/evaluationData.json");
    ...
    {
        var CoherenceBar = barChart.AddSerie<Bar>("Coherence", true);
        var RelevanceBar = barChart.AddSerie<Bar>("Relevance", true);
        var NaturalnessBar = barChart.AddSerie<Bar>("Naturalness", true);
        var EngagementBar = barChart.AddSerie<Bar>("Engagment", true);
        var CABar = barChart.AddSerie<Bar>("Contextual Accuracy", true);

        serieBars.Add(CoherenceBar);
        serieBars.Add(RelevanceBar);
        serieBars.Add(NaturalnessBar);
        serieBars.Add(EngagementBar);
        serieBars.Add(CABar);
        ...
        foreach (var data in evaluationdataList.dataList)
        {
            i++;
            barChart.AddXAxisData(i.ToString());
            barChart.AddData(CoherenceBar.index, float.Parse(data.CoherenceScore));
            barChart.AddData(RelevanceBar.index, float.Parse(data.RelevanceScore));
            barChart.AddData(NaturalnessBar.index, float.Parse(data.NaturalnessScore));
            barChart.AddData(EngagementBar.index, float.Parse(data.EngagementScore));
            barChart.AddData(CABar.index, float.Parse(data.ContextualAccuracyScore));
        }
    }
