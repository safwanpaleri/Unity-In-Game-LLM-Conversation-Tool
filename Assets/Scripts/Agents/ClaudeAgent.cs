...
public class ClaudeAgent : MonoBehaviour
{
    ...
    {
        public RequestContentTTS[] contents;
        public RequestConfig generationConfig;
        public string model;
    }

    [System.Serializable]
    public class RequestContentTTS
    {
        public Part[] parts;
    }
    ...
    private List<RequestContent> requestContents = new List<RequestContent>();
    ...
    private void Start()
    {
        ...
        // Initial instruction for claude to behave as a specific character in group conversation
        initialContent = "act as a " + characterDescription + ", named as " + characterName +
            " , and talk about " + topic + ". generate the dialogue like the example given \"Say Confused: the data shows people are choosing veganism, I wonder why\" \"Say happy: I have passed the test!\" \"Say imitating: Let the bygones be bygones\" only give one dailogue. This is group conversation but only give dailogues for" + characterName +
            "the other characters are " + otherCharactersNames + ". and only include emotion and dailogue no action just like the examples, also don't include the speaker name at start";
    }
    ...
    /// <summary>
    /// Public function to call from any script to generate dailogue using Claude.
    /// </summary>
    ...
    public void GenerateNextDailogue(List<string> conversationHistory, string additionalContent = "")
    {
        ...
        requestContents.Add(new RequestContent { role="user", content= initialContent + '\n' + AdditionalPrompt });
        foreach (var conversation in conversationHistory)
        {
            requestContents.Add(new RequestContent { role = "user", content = conversation });
        }
        if(additionalContent != "")
            requestContents.Add(new RequestContent { role="user", content = additionalContent });
    ...
    private IEnumerator SendRequest()
    {
        ...
        requestBody.messages = requestContents.ToArray();
    ...
    IEnumerator SpeakWithGemini()
    {
        ...
        RequestBodyTTS requestBodyTTS = new RequestBodyTTS();
        RequestContentTTS[] requestContentsTTS = new RequestContentTTS[1];
        requestContentsTTS[0] = new RequestContentTTS();
        requestContentsTTS[0].parts = new Part[1];
        requestContentsTTS[0].parts[0] = new Part { text = textToSpeak };
        requestBodyTTS.contents = requestContentsTTS;
        requestBodyTTS.generationConfig = new RequestConfig
        {
        ...
        requestBodyTTS.model = "gemini-2.5-flash-preview-tts";

        string jsonData = JsonUtility.ToJson(requestBodyTTS);
        byte[] bodyRaw = Encoding.UTF8.GetBytes(jsonData);
    ...
    public void GenerateModeratorContent(string content)
    {
        ...
        isCompleted = false;
        requestContents.Clear();
        requestContents.Add(new RequestContent { role = "user", content = content });
        StartCoroutine(SendRequest());
    }
