...
public class ChatGPTAgent : MonoBehaviour
{
    ...
    {
        public string model;
        public APIRequestContent[] messages;
    }
    ...
    {
        public APIRequestContent message;
    }
    ...
    [HideInInspector] public List<APIRequestContent> conversationContents = new List<APIRequestContent>();
    ...
    /// <summary>
    /// Public function to call from any script to generate dailogue using ChatGpt.
    /// </summary>
    ...
    public void GenerateNextDailogue(List<string> conversationHistory, string additionalContent = "")
    {
        ...
        conversationContents.Add(new APIRequestContent { role = "user", content = initialContent + '\n' + AdditionalPrompt });
        foreach (var conversation in conversationHistory)
        {
            conversationContents.Add(new APIRequestContent { role = "user", content = conversation });
        }
        if (additionalContent != "")
            conversationContents.Add(new APIRequestContent { role = "user", content = additionalContent });
    ...
    private IEnumerator SendRequest()
    {
        ...
        requestBody.messages = conversationContents.ToArray();
    ...
    /// <summary>
    /// A public function to convert any text to speech.
    /// First have to assign the text to the public variable textToSpeak
    /// and then call this function. 
    /// </summary>
    public void TextToSpeech()
    {
        //Calls the coroutine to convert the text to speech.
        StartCoroutine(SpeakWithGemini());
    }
    ...
    public void GenerateModeratorContent(string Content)
    {
        ...
        isCompleted = false;
        conversationContents.Clear();
        conversationContents.Add(new APIRequestContent { role="user", content=Content });
        StartCoroutine(SendRequest());
    }
